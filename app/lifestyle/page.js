...
const Lifestyle = async({searchParams}) => {
  ...
  return (
    <section className='w-full h-full flex flex-col items-center py-4 md:py-8 px-4 md:px-[4rem] lg:px-[8rem]'>
      ...
      {
        lifestyles.length===0?<div className="w-full h-screen flex items-center justify-center">
        ...
        <Link
          ...
          className={`py-2 px-6 bg-black text-white text-sm rounded border hover:border-black hover:bg-white hover:text-black font-semibold ${lifestyles.length===0?"pointer-events-none opacity-50":""}`}
        >
          Next
        </Link>
      </div>
